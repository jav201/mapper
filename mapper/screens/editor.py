...
class EditorScreen(ModalScreen[str | None]):
    ...
    def _detected_line(self, source: str) -> Text:
        ...
        for i, tag in enumerate(tags):
            ...
            parts.append((escape(tag), darkside.ACCENT))
        if not tags:
    ...
    def _render_preview(self, source: str) -> Text:
        ...
        for token in _TAG_RE.split(source):
            if _TAG_RE.fullmatch("{{" + token + "}}"):
                parts.append((escape("{{" + token + "}}"), darkside.ACCENT))
            else:
                parts.append((escape(token), darkside.INK))
        return Text.assemble(*parts)
