...
| B-31 | **⚠ MECHANISM WITHDRAWN 2026-08-27 — carried as an UNVERIFIED OBSERVATION, not a diagnosed defect.** The security lens could not reproduce the stated empty-delta mechanism: `insp-title`'s value *did* change, `insp-notes` did *not* rewrite, and the empty-delta control is **0 of 8**. Four writes against one value change over eight focusables means *something* writes without a ficha delta, but it is **not** the empty-delta path and it was not isolated. **Do not inherit the mechanism as fact** — a fix built for it would target a defect nobody has demonstrated. Original claim, retained for the record: **`_commit` rewrites the sidecar on blur even when the delta is EMPTY.** `insp-title` and `insp-notes` both show `SIDECAR REWRITTEN=True` with no value change — a durable disk write for a no-op edit | Follow-on **design** batch, with US-N14 «lente» — same confirmation-affordance ruling as the data-loss family |
...
| B-33 | **`S-15` / `M-H3` — `MAX_RENDER_NODES` bounds the COUNT and not the WORK, and the gap is app-killing.** Twice measured: a **73-node** map costs **72.5 s** and the 12 000-node limit waves it straight through, because cost is driven by EDGES (the 51-node fixture carries 410) while the cap counts nodes. Three independent copies of the constant (`mapper/views/layered.py:15`, `outline.py:14`, `radial.py:28`) over three private walks, and `children_of` is still O(E) per call (`mapper/model.py:149-150`) | Follow-on **design** batch, paired with `S-18`+`S-19`. **Recorded here so "C-3 disposed" is never readable as "the defect is fixed" — it is LIVE on `master` today** |
| B-34 | **Two decision registries collide on one id grammar, so a bare `Dn` is ambiguous batch-wide.** `PLAN.md:244` `D10` is the Q-3 **seat rebind**; the sealed `PDR-2026-08-26-ui-next-batch-02.md:566` `#D10` is the Q-10 **hue dispositions**. The PDR's own prose at `:372`, `:396` and `:484` writes bare `D10` meaning the PLAN's. This produced a routed instruction to "re-open `#D10`" naming the wrong artifact's decision; the seat pin's real carrier is `#D5b`. Verified independently by the architect lens | `PDR-addendum-3` §1. **Remedy: one namespace, or prefix every citation.** C-50 — the ids are the glue of the repo/vault split, and an id resolving to two decisions is the glue failing |
| B-35 | **The legend's CONTENT-discoverability is unclosed: at rest nothing declares that pages 2 and 3 exist.** `#D26` rules the legend scrolls this batch and defers the two-pane/tabbed redesign; `HLR-N16.4` closes **key**-discoverability and deliberately not content-discoverability | `PDR-addendum-3#D26` + `C-D26c`. **Inherit the measured `72 rows / 24-row viewport ≈ 3 pages`, NOT the "~44 rows" the routing carried** — the ux lens measured the viewport at 24, not the terminal's 34, so the flat panel is out by a wider margin than `01b` §3.8 claimed |
| B-36 | **`UX2-C-01` — one keystroke durably overwrites a map and its sidecar, no confirmation, no explicit edit gesture. LIVE ON `master` THROUGH THIS BATCH, BY DECISION.** `mapper/widgets/inspector.py:277-278` calls `_commit` unconditionally on blur; `_commit` (`:280-291`) posts `FieldCommitted(..., widget.value)` regardless of delta. Measured: **7 of 8** focusables overwrite from one real `n` press; both `.mmd` and `.yml` rewritten. It has already fired once on this repository's own tracked fixtures. **Two cheap remedies were EXECUTED and BOTH FAIL:** the non-empty-delta gate is already implemented at `mapper/app.py:1393-1395` and is invariant under the defect (after the keystroke the delta is genuinely non-empty); clearing `select_on_focus` converts destruction into corruption, still 7/7 written to disk. **A design ruling with no cheap fix, not a cheap fix postponed.** Candidate real remedies: a dirty-since-focus flag (`Input.Changed`), or dropping `on_input_blurred` and keeping the shipped `on_input_submitted` (`:275`) | All four PDR-3 lenses ruled explicitly. Deferral of the **affordance design** accepted because `inspector.py` is in no live increment's source budget, so this batch neither touches nor worsens it; the **data loss** is carried with its reproduction and its two failed remedies recorded, so the follow-on batch inherits a defect and not a discovery. **OPERATOR RIDERS 2026-08-27:** (a) named in the batch's final packet risks **verbatim** — *"one keystroke, no confirmation, permanent overwrite, already fired on this repo's own fixtures"*; (b) **FIRST scope item of the follow-on design batch, AHEAD of «lente»** — a live data-loss defect outranks a new feature in any queue |
| B-37 | **Evaluate recalibrating `MUT`'s hex to clear `4.5 : 1` on `PANEL`.** `#D28` ruled the contrast floor stands and the token changes, so `V7`/`V8` and the damaged-card glyph escalate to `INK`. That is correct per-seat but **erodes the hierarchy ladder** if more readable-text seats appear on `PANEL`: measured `MUT` on `GROUND` `4.43 : 1` (legal) vs on `PANEL` **`3.57 : 1`** (below floor), and `EIGHT_BIT` slot 242 `4.00 : 1`. A recalibrated `MUT` would let the dim tier carry readable text on `PANEL` again | `PDR-addendum-3#D28` item 4. **Cost stated honestly, and it is why this is NOT done mid-batch: a `MUT` change reddens byte-identity pins tree-wide.** Decorative marks (rules, lattice dots, ground texture) are exempt from the floor and unaffected |
